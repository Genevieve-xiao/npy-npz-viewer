"""
NPY/NPZ 数据查看器 v2.2 - 优化布局版
集成折叠组和滚动条，解决左侧面板过长问题
"""
import sys
...
from core.visualization_3d import Visualizer3D
from utils.helpers import PreviewHelper
from utils.large_data_optimizer import LargeDataOptimizer
from ui.optimized_left_panel import OptimizedLeftPanel
from ui.custom_preview import CustomPreviewWidget
from ui.task_runner import TaskRunner


class MainWindowV22(QMainWindow):
    """v2.2 主窗口 - 优化布局"""
    ...
    def init_ui(self):
        self.setWindowTitle("NPY/NPZ 数据查看器 v2.2 - 优化布局版")
    ...
    def get_supported_drop_file(self, event):
        ...
        for url in event.mimeData().urls():
            ...
            if file_path.lower().endswith(('.npy', '.npz')):
                return file_path
        return None
    ...
    def dropEvent(self, event):
        ...
        if not file_path:
            QMessageBox.warning(self, "不支持的文件", "请拖入 .npy 或 .npz 文件")
            event.ignore()
            return
        ...
        supported_count = sum(
            1
            for url in event.mimeData().urls()
            if url.isLocalFile() and url.toLocalFile().lower().endswith(('.npy', '.npz'))
        )
    ...
    def on_npz_key_selected(self, key: str):
        ...
        def switch_payload(worker):
            worker.progress.emit("切换 NPZ 数组", 20, f"读取 {key}")
            payload = loader.switch_npz_key(key)
            payload['load_id'] = load_id
            payload['loader'] = loader
            worker.progress.emit("切换 NPZ 数组", 90, "准备数组视图")
            return payload

        self.task_runner.run_task("切换 NPZ 数组", switch_payload)
    ...
    def schedule_view_refresh(self, array: np.ndarray, include_stats: bool):
        ...
        def build_payload(worker):
            ...
            df, info = PreviewHelper.build_preview(array, mode, start, end)
            metadata = {
    ...
    def plot_sequence(self, plot_type: str) -> dict:
        ...
        if LargeDataOptimizer.should_downsample_1d(array):
            downsampled, info = LargeDataOptimizer.downsample_1d(array)
    ...
    def plot_tabular(self, plot_type: str, params: dict) -> dict:
        ...
        if plot_type == "多折线图":
            ...
            downsampled, x_downsampled, info = LargeDataOptimizer.downsample_2d_for_plot(
                array, x_data
            )
            ...
            x_data = array[:, x_col]
            y_data = array[:, y_col]
            x_down, y_down, info = LargeDataOptimizer.downsample_scatter(x_data, y_data)
            ...
            return SemanticVisualizer.plot_tabular_correlation(
                array, self.figure, column_labels=self.get_column_labels(array.shape[1])
            )
    ...
    def plot_image(self, plot_type: str, params: dict = None) -> dict:
        ...
        if array.ndim == 2:
            downsampled, info = LargeDataOptimizer.downsample_heatmap(array)
            if info:
                self.downsample_info.setText(info)
                self.downsample_info.show()
            array = downsampled
    ...
    def export_csv(self):
        ...
        try:
            ...
            if self.current_array.ndim == 2:
                df.columns = self.get_column_labels(self.current_array.shape[1])
    ...
    def make_quick_2d_view(self):
        ...
        view = np.squeeze(array)
        fixed = []
        while view.ndim > 2:
            axis = view.ndim - 1
            index = view.shape[axis] // 2
            view = np.take(view, index, axis=axis)
...
def main():
    ...
    app.setApplicationName("NPY/NPZ 数据查看器 v2.2")
    app.setOrganizationName("DataViewer")

    window = MainWindowV22()
    window.show()
