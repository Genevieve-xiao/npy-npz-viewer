...
from ui.universal_slice import UniversalSliceWidget
...
class MainWindowV21(QMainWindow):
    ...
    def __init__(self):
        ...
        self.original_array = None  # 原始数组（未切片）
        self.current_array = None   # 当前数组（可能已切片）
        self.current_semantic = None
        self.semantic_info = None
        self.init_ui()

    def init_ui(self):
        self.setWindowTitle("NPY/NPZ 数据查看器 v2.1 - 大数据优化版")
    ...
    def create_left_panel(self) -> QWidget:
        ...
        self.slice_widget.slice_applied.connect(self.on_slice_applied)
        ...
        self.semantic_control.plot_requested.connect(self.on_plot_requested)
    ...
    def on_slice_applied(self, sliced_array: np.ndarray):
        """🆕 切片应用后的回调"""
        self.current_array = sliced_array

        # 更新统计信息
        stats = ArrayStats.compute_stats(sliced_array)
        stats_text = ArrayStats.format_stats(stats)
        self.stats_text.setText(stats_text)

        # 更新语义控制
        self.semantic_info = SemanticInference.infer(sliced_array)
        self.current_semantic = self.semantic_info['semantic']
        self.semantic_control.set_array_info(sliced_array.shape, self.semantic_info)

        # 更新预览
        self.update_preview(sliced_array)
    ...
    def plot_tabular(self, plot_type: str, params: dict) -> dict:
        ...
        if plot_type == "多折线图":
            ...
            return SemanticVisualizer.plot_tabular_correlation(array, self.figure)
    ...
    def plot_image(self, plot_type: str) -> dict:
        ...
        if plot_type in ["热力图", "图像显示"]:
            return SemanticVisualizer.plot_image_heatmap(array, self.figure)
        else:
            return {'success': False, 'error': f'未知图表类型: {plot_type}'}
...
def main():
    ...
    app.setApplicationName("NPY/NPZ 数据查看器 v2.1")
