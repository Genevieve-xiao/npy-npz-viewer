...
class Visualizer3D:
    ...
    def plot_3d_voxel(array: np.ndarray, figure: Figure,
        ...
        try:
            d_idx = Visualizer3D._regular_indices(array.shape[0], 60)
            h_idx = Visualizer3D._regular_indices(array.shape[1], 60)
            w_idx = Visualizer3D._regular_indices(array.shape[2], 60)
            sampled = array[np.ix_(d_idx, h_idx, w_idx)]
