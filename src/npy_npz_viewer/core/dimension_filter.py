...
class DimensionFilter:
    ...
    @staticmethod
    def _squeeze_axis(array, axis: int):
        if hasattr(array, "squeeze"):
            return array.squeeze(axis=axis)
        return np.squeeze(array, axis=axis)

    @staticmethod
    def _take_axis(array, indices: List[int], axis: int):
        if hasattr(array, "take"):
            return array.take(indices, axis=axis)
        ...
        for raw_filter in axis_filters:
            ...
            if mode == "drop":
                if len(selected) == size:
                    ...
                    filtered = DimensionFilter._squeeze_axis(filtered, current_axis)
            ...
            try:
                filtered = DimensionFilter._take_axis(filtered, final_indices, current_axis)
